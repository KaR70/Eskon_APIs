...
/// <param name="cancellationToken">Cancellation token for the async operation.</param>
    ...
    public async Task<IActionResult> Create([FromBody] AmenityRequest amenityRequest, CancellationToken cancellationToken = default)
    {
    var addedAmenity = await _amenityService.CreateAsync(amenityRequest, cancellationToken);

        return CreatedAtAction(nameof(Get), new { id = addedAmenity.Value.AmenityId }, addedAmenity);
 }
